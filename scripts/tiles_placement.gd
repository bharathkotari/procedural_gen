...
func _process(delta):
	...
	if(i==0):
		...
		fill_ground(i,prev)
	elif(ra<prev)and(i-1>1) and (map1.get_cell(i-2,prev)==top_tile)and (prev-1>=0):
		...
		prev=prev+1
		fill_ground(i,prev)
	elif(ra>prev)and(i-1>0)and(map1.get_cell(i-2,prev-1)==top_tile)and(prev-1>=0):
